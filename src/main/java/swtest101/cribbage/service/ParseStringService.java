...
public class ParseStringService {
	...
	public static final String INVALID_CARDS_SIZE_ERROR_MSG = "Input String is invalid. It should have ten characters: ";
	...
	public static final String NULL_ARGUMENT_PARSE_ERROR_MSG = "Parametro Null passado!!!";
	...
	public static Card parseToCard(String cardStr) {
		...
		if (cardStr.length() != 2) {
			throw new InputParseException(INVALID_CARD_STRING_ERROR_MSG);
		}
	...
	public static List<String> parseToListOfStringCards(String handOfCards) {
		
//		if(handOfCards == null) {		
//			throw new NullArgumentException(NULL_ARGUMENT_PARSE_ERROR_MSG);
//			
//		} else 
			if (handOfCards.length() != 10) {
			throw new InputParseException(INVALID_CARDS_SIZE_ERROR_MSG + handOfCards.length());
		}
		
		List<String> stringsCard = new ArrayList<String>();		
		
		for (int i=0; i < handOfCards.length(); i+=2) {
			String carta = handOfCards.substring(i, i+2);
			stringsCard.add(carta);
		}
		return stringsCard;
	}
	...
	public static List<Card> parseToListOfCards(String handOfCards) {
		...
		List<String> strCards = parseToListOfStringCards(handOfCards);
		List<Card> cards = new ArrayList<Card>();
		
		Card card = null;
		for (String strCard : strCards) {
			card = parseToCard(strCard);
			cards.add(card);
		}
